// Platform -> Source -> Network -> Core -> RemoteClient -> EndpointBuilder
...
extension EndpointBuilder {
  func send<T: Decodable>(endpoint: Endpoint) async throws -> T {
    guard
      let urlComponent = URLComponents.generate(baseURL: endpoint.url, parameter: endpoint.parameter),
...
extension URLComponents {
  fileprivate static func generate(baseURL: String, parameter: QueryParameterType) -> Self? {
    var componet = URLComponents(string: baseURL)
    componet?.queryItems = parameter.makeQuery().map {
